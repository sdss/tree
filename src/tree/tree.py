...
from tree import config as cfg_params
...
class Tree(object):
    ...
    def __repr__(self):
        return "Tree(sas_base_dir={0}, config={1}, release={2})".format(
            self.sasbasedir, self.config_name, self.release
        )
    ...
    def _read_config(self, config=None, bases=None):
        ...
        assert os.path.isfile(config_file) is True, (
            "config file {0} must exist in the data directory".format(config_file)
        )
        ...
        if config_name not in file_base:
            if "dr" in config_name:
                drint = max(
                    map(
                        int,
                        [re.findall(r"\d{1,2}", f)[0] for f in file_base if "dr" in f],
                    )
                )
    ...
    def load_config(self, config=None):
        ...
        assert os.path.isfile(configfile) is True, (
            "configfile {0} must exist in the data directory".format(configfile)
        )
    ...
    def _create_environment(self, cfg=None, sections=None):
        ...
        filesystem = (
            "FILESYSTEM"
            if "FILESYSTEM" in environ["default"]
            else "filesystem"
            if "filesystem" in environ["default"]
            else None
        )
    ...
    def get_paths(self, key):
        ...
        """
        newkey = (
            key if key in self.environ else key.upper() if key.upper() in self.environ else None
        )
        if newkey:
    ...
    def _sort_configs(release: str = "dr", cfgs: list = None) -> list:
        ...
        relsort = sorted(
            [i for i in cfgs if release in i],
    ...
    def get_product_root(root=None, git=None):
        ...
        product_root = os.getenv(
            repo_root,
            os.getenv("SDSS_INSTALL_PRODUCT_ROOT", os.getenv("SDSS4_PRODUCT_ROOT", None)),
        )
        if not product_root:
    ...
    def write_old_paths_inifile(self, no_pipe=None):
        ...
        with open(paths_dir, "w") as f:
            ...
            f.write(
                "# It should not be updated manually.  Use the tree.write_olds_paths method in \n"
            )
