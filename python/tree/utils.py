...
def compute2_changelog(new, old, pprint=None, remove_sas=True, include_paths=True):
    ...
    if include_paths:
        ...
        diffs.append('-----------------')
        path_changes = compute_path_changes(new, old)
...
def compute_path_changes(new, old, pprint=None, prepend_header=None):
    ''' Compute the difference between two Tree PATH sections

    Finds and prints the difference between two tree PATH ini sections from
    the given environment configurations.  Accepts either string names of config
    files, e.g. "dr16" and "dr15", or the preloaded `Tree` configs, e.g.
    `Tree(config='dr16')`.  Indicates new `sdss_access` paths, as well as any changes
    to existing paths, between the two prodived releases.
    ...
    Example:
        >>> # print the differences DR16 and DR15
        >>> diffs = compute_path_changes('dr16', 'dr15', pprint=True)
        >>> print(diffs)
    '''
    ...
    if unique_new:
        ...
        for path in unique_new:
            diffs.append('{0}: {1}'.format(path, new_templates[path]))
    ...
    if changed:
        ...
        for item in changed:
            ...
            diffs.append('   from: {0}'.format(oldt))
            diffs.append('   to: {0}'.format(newt))
...
def compute_changelog(new, old, pprint=None, to_list=None, remove_sas=True, include_paths=True):
    ...
    cl_dict = _compute_environment(new, old, remove_sas=remove_sas)

    # compute and add the paths changelog dictionary
    if include_paths:
        path_dict = _compute_paths(new, old)
...
def _compute_environment(new, old, remove_sas=True):
    ''' Compute the difference between two Tree environments
...
def _compute_paths(new, old, prepend_header=None):
    ''' Compute the difference between two Tree PATH sections
